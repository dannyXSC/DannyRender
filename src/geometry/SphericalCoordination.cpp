#include "SphericalCoordination.hpp"
    ...
    namespace geometry
    {
        SphericalCoordinate::SphericalCoordinate(const glm::vec3 &cartesian_coordinate)
            : radius(glm::length(cartesian_coordinate)),
              theta(glm::atan(glm::sqrt(cartesian_coordinate.x * cartesian_coordinate.x + cartesian_coordinate.y * cartesian_coordinate.y),
                              cartesian_coordinate.z)),
        ...
        SphericalCoordinate::SphericalCoordinate(float p_radius, float p_theta, float p_phi)
            : radius(p_radius), theta(p_theta), phi(p_phi)
        {
        }
